...
public class NSGAIII_DE extends Algorithm {
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		int FEsRecord_ = ((Integer) this.getInputParameter("FEsRecord")).intValue();
		...
		indicators = new QualityIndicator(getProblem(),
				"E:\\new_multiobjective\\jMetal\\Pareto_front\\" + testName + ".dat");
		// System.out.println(getProblem().getName());
		...
		while (generations_ < maxGenerations_) {
			...
			for (int i = 0; i < (populationSize_ / 2); i++) {
				if (generations_ < maxGenerations_) {
					...
					Solution offSpring1 = (Solution) crossover_.execute(new Object[] { parents2[0], parents });
					Solution offSpring2 = (Solution) crossover_.execute(new Object[] { parents2[0], parents });
					...
					if (fes % FEsRecord_ == 0 && fes != maxGenerations_ * populationSize_) {
